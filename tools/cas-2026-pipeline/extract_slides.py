...
def _ppt_to_pdf(src: Path, tmp_dir: Path) -> Path:
    ...
    Note: If your PowerPoint install requires the app to be visible to open files,
    set `ppt_app.Visible = True` before the `Presentations.Open` call.
    """
    ...
    try:
        try:
            ...
            raise RuntimeError(f'PowerPoint failed to open {src.name}: {e}') from e
        ...
        try:
            ...
            # Don't let a hung/erroring Quit() block the batch.
            print(f'  WARN PowerPoint Quit() failed: {e}', file=sys.stderr)
    return out

def _pdf_to_pngs(pdf_path: Path, out_dir: Path, dpi: int = 300) -> list[Path]:
    """Render each PDF page to a PNG in out_dir.

    The document is closed via context manager so handles do not leak on exception.
    """
